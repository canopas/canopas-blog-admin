          ...
          >
            <Link href={"/" + post.slug}>
              <div className="flex flex-row space-x-2 items-center text-base md:text-base leading-5 tracking-normal capitalize">
                ...
                <span className="font-inter-medium">{post.authorName}</span>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-10 md:gap-12 xl:gap-16 mt-4">
                <div className="col-span-2">
                  <div
                    className="mb-2 lg:mb-3.5 text-[1.1875rem] md:text-[1.40625rem] xl:text-[1.625rem] leading-7 md:leading-snug tracking-none line-clamp-2 lg:line-clamp-4 font-inter-semibold"
                    onClick={() => {
                ...
                {post.tags.map((tag) => {
                  return (
                    <div className="my-2.5 mr-2" key={tag.id}>
                      <Link
                        href={"/tag/" + tag.slug}
                        className="my-1 rounded-full bg-[#f2f2f2] px-2 py-1.5 font-medium no-underline capitalize"
                      >
                        {tag.name}
                      </Link>
                    </div>
